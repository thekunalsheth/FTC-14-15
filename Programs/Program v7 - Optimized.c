...
void grabberRun(){
	if(grab==true){grabber(MED);}
	else{grabber(0);}}

void leftHigh(){LMulti=-0.8;}
void leftLow(){LMulti=-0.3;}
void rightHigh(){RMulti=-0.8;}
void rightLow(){RMulti=-0.3;}
...
task main(){
	while(true){
		...
		if(joystick.joy1_TopHat==0){forward(-VERYSLOW);}
			...
			if(abs(joystick.joy1_y1)>10){left(joystick.joy1_y1*LMulti);}
			else if(joystick.joy1_TopHat==6){left(-VERYSLOW);}
			else{left(0);}
			if(abs(joystick.joy1_y2)>10){right(joystick.joy1_y2*RMulti);}
			else if(joystick.joy1_TopHat==2){right(-VERYSLOW);}
			else{right(0);}}

		//Rack Motors
		if(abs(joystick.joy2_y1)>10){rackOne(joystick.joy2_y1*MULTI);}
		else if(joy1Btn(3)==1||joystick.joy2_TopHat==5){rackOne(VERYFAST);}
		else if(joy1Btn(4)==1||joystick.joy2_TopHat==7){rackOne(-VERYFAST);}
		else{rackOne(0);}
		if(abs(joystick.joy2_y2)>10){rackTwo(joystick.joy2_y2*MULTI);}
		else if(joystick.joy2_TopHat==3){rackTwo(VERYFAST);}
		else if(joystick.joy2_TopHat==1){rackTwo(-VERYFAST);}
		else{rackTwo(0);}
		...
		//Setting Boost
		if(joy1Btn(9)==1){leftHigh();}
		else{leftLow();}
		if(joy1Btn(10)==1){rightHigh();}
		else{rightLow();}
